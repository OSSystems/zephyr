		   ...
		   void *ip)
{
	...
	char hr_addr[NET_IPV6_ADDR_LEN], *hr_family, *buffer;
	int buffer_len = 0;
	...
	case DNS_EAI_INPROGRESS:
		if (info && info->ai_family == AF_INET) {
			...
			memcpy(ip, buffer, MIN(MAX_IP_SIZE, buffer_len));
		}
...
static bool start_coap_client(void)
{
	...
	k_sem_take(&ctx.semaphore, NETWORK_TIMEOUT * 2);

	if (net_addr_pton(addr.sin_family, ip, &addr.sin_addr)) {
...
static void install_update_cb(void)
{
	...
	char buffer[2], sha256_image_dowloaded[TC_SHA256_BLOCK_SIZE + 1];
	...
	int rcvd = recv(ctx.sock, data, MAX_DOWNLOAD_DATA, MSG_DONTWAIT);
	if (rcvd <= 0) {
...
static int report(enum updatehub_state state)
{
	...
	char *device_id = k_malloc(DEVICE_ID_SIZE);
	...
	if (!updatehub_get_device_identity(device_id)) {
		goto error;
	}
	...
	if (strncmp(report.previous_state, "", strlen("")) != 0) {
...
static void probe_cb(char *metadata)
{
	...
	int rcvd = recv(ctx.sock, metadata, MAX_PAYLOAD_SIZE, MSG_DONTWAIT);
	if (rcvd <= 0) {
		LOG_ERR("Could not receive data");
		ctx.code_status = UPDATEHUB_NETWORKING_ERROR;
		return;

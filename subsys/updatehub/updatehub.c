...
static int storage_overwrite_ip()
{
	...
	LOG_INF("No address found, adding overwrite_ip at id %d",
		ADDRESS_ID);
...
static void install_update_cb()
{
	...
	rcvd = recv(sock, data, MAX_DOWNLOAD_DATA, MSG_DONTWAIT);
	if (rcvd <= 0) {
	...
	updatehub_ctx.downloaded_size = updatehub_ctx.downloaded_size +
					(response_packet.max_len - response_packet.offset);
	ret = tc_sha256_update(&sha256sum,
	...
	if (updatehub_ctx.downloaded_size ==
	    updatehub_ctx.block.total_size) {
...
static enum updatehub_response install_update()
{
	...
	ret = coap_block_transfer_init(&updatehub_ctx.block,
				       COAP_BLOCK_1024,
				       _probe.image_size);
	if (ret < 0) {
...
static void probe_cb(char *metadata)
{
	...
	rcvd = recv(sock, metadata, MAX_PAYLOAD_SIZE, MSG_DONTWAIT);
	if (rcvd <= 0) {
		LOG_ERR("Could not receive data");
		updatehub_ctx.code_status = UPDATEHUB_NETWORKING_ERROR;
		goto end;
...
static enum updatehub_response probe()
{
	...
	ret = json_obj_encode_buf(send_probe_descr,
				  ...
				  &probe_send, updatehub_ctx.payload,
				  MAX_PAYLOAD_SIZE - 1);
	if (ret < 0) {
		LOG_ERR("Could not encode metadata");
		updatehub_ctx.code_status = UPDATEHUB_METADATA_ERROR;
	...
	if (ret < 0) {
		...
		if (ret < 0) {
			...
			updatehub_ctx.code_status = UPDATEHUB_INCOMPATIBLE_HARDWARE;
			goto cleanup;
		}
		memcpy(_probe.sha256sum_image,
...
#if defined (CONFIG_UPDATEHUB_SHELL)
static int cmd_updatehub_run(const struct shell *shell,
